...
这是一个基于 Go 实现的后端应用，用于承载 go-label-modularbuild-g01 的业务处理、数据管理与运行维护。
...
1. 预期退出码 0：`go test ./internal/booking -run '^TestCancellationDuringReservationDoesNotConsumeCapacity$' -count=1`
2. 预期退出码 0：`go test ./...`
